...
test.describe("package creation flow", () => {
  ...
  test("fills all form fields and creates a new package", async ({ page }) => {
    ...
    const titleInput = page.locator(SEL.title);
    await titleInput.clear();
    await titleInput.pressSequentially("Playwright Test Package", { delay: 30 });

    const slugInput = page.locator(SEL.slug);
    await slugInput.clear();
    await slugInput.pressSequentially(`playwright-test-package-${Date.now()}`, { delay: 30 });
    ...
    await page.locator(SEL.itinerary).fill(
    ...
    expect(afterUrl).toContain("/dashboard/packages");
    expect(afterUrl).not.toContain("/new");
  });
});
